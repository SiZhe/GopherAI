...
import (
	...
	"GopherAI/common/mysql/dao/session"
...
func GetUserSessionsByUserName(userName string) ([]model.SessionInfo, error) {
	//获取用户的所有会话ID
	sessions := aihelper.GetGlobalManager().GetUserSessions(userName)
...
func CreateGenerateSessionAndSendMessage(userName string, userQuestion string, modelType string) (string, string, code.Code) {
	//1：创建一个新的会话
...
func GetChatHistory(userName string, sessionID string) ([]model.History, code.Code) {
	// 获取AIHelper中的消息历史
	helper, exists := aihelper.GetGlobalManager().GetAIHelper(userName, sessionID)
	if !exists {
